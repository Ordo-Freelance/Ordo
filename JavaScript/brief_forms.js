(function(){
  ...
  function questionMarkup(q){
    ...
    return '<div class="brief-editor-block brief-question" data-id="'+esc(q.id||id())+'">'+
      ...
      '<div class="brief-options" style="'+(['radio','checkbox','select'].includes(q.type)?'':'display:none')+'"><small>كل سطر اختيار مستقل</small><textarea class="form-textarea brief-q-options" rows="3" placeholder="اختيار ١&#10;اختيار ٢">'+esc((q.options||[]).map(function(o){return typeof o==='string'?o:(o.label||'');}).join('\n'))+'</textarea></div>'+
  ...
  window.createProposalFromBrief=function(formId){
    ...
    if(!form||!client||typeof window.openProposalModal!=='function')return;
    ...
    openProposalModal();
    var clientEl=document.getElementById('prop-client');if(clientEl)clientEl.value=client.name||'';
    ...
    document.getElementById('prop-notes').value=[pricing.duration?'مدة التنفيذ: '+pricing.duration:'',pricing.revisions?'جولات التعديل: '+pricing.revisions:'',pricing.extraDeliverable?'سعر المخرج الإضافي: '+pricing.extraDeliverable:'',pricing.extraRevision?'سعر الجولة الإضافية: '+pricing.extraRevision:''].filter(Boolean).join('\n');
