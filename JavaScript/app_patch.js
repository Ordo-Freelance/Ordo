...
function _normalizeSectionState(value) {
  if(value && typeof value === 'object') {
    if(value.state !== undefined) return _normalizeSectionState(value.state);
    if(value.status !== undefined) return _normalizeSectionState(value.status);
    if(value.enabled === false) return 'disabled';
    if(value.coming === true || value.comingSoon === true || value.coming_soon === true) return 'coming';
    if(value.enabled === true) return 'enabled';
  }
  if(value === false) return 'disabled';
  if(value === true || value === undefined || value === null || value === '') return 'enabled';
  const v = String(value).toLowerCase().replace(/[\s-]+/g, '_');
  if(['disabled','disable','hidden','hide','off','false','locked','closed'].includes(v)) return 'disabled';
  if(['coming','soon','coming_soon','upcoming','launch','prelaunch'].includes(v)) return 'coming';
  return 'enabled';
}

function _sectionAliases(sectionId) {
  const map = {
    tasks: ['tasks','projects'],
    goals: ['goals','freelancer-goals','learning'],
    'freelancer-goals': ['freelancer-goals','learning','goals'],
  ...
  const list = map[sectionId] || [sectionId];
  return Array.from(new Set([sectionId].concat(list)));
}

function _collectSectionNav(sectionId) {
  const aliases = _sectionAliases(sectionId);
  const found = [];
  document.querySelectorAll('.nav-item, .bn-item, .bn-more-item').forEach(el => {
    const oc = el.getAttribute('onclick') || '';
    const id = el.id || '';
    if(aliases.some(a => oc.includes("'" + a + "'") || oc.includes('"' + a + '"') || id === 'nav-' + a || id === 'bn-' + a || id === 'bnm-' + a)) found.push(el);
  });
  return found;
}

function _setComingBadge(el, enabled) {
  if(!el) return;
  el.querySelectorAll('.admin-coming-badge').forEach(b => b.remove());
  if(!enabled) return;
  const badge = document.createElement('span');
  badge.className = 'admin-coming-badge';
  badge.textContent = 'قريباً';
  badge.style.cssText = 'margin-inline-start:auto;font-size:10px;font-weight:900;color:var(--accent2);background:rgba(247,201,72,.12);border:1px solid rgba(247,201,72,.24);border-radius:999px;padding:1px 6px;white-space:nowrap';
  el.appendChild(badge);
}

window._getSectionAdminState = function(sectionId) {
  const cfg = window._adminSectionsConfig || {};
  const aliases = _sectionAliases(sectionId);
  for(let i = 0; i < aliases.length; i++) {
    if(Object.prototype.hasOwnProperty.call(cfg, aliases[i])) return _normalizeSectionState(cfg[aliases[i]]);
  }
  return 'enabled';
};

window._isSectionEnabled = function(sectionId) {
  if(window._getSectionAdminState(sectionId) !== 'enabled') return false;
  if(typeof hasPageFeature === 'function' && !window._isAdminUser) return hasPageFeature(sectionId);
  return true;
};

window._isFeatureEnabled = function(featureId) {
  if(_normalizeSectionState((window._adminSectionsConfig || {})[featureId]) === 'disabled') return false;
...
async function _loadSectionsConfig() {
  try {
    ...
    if(data?.config) {
      ...
      window._adminSectionsConfig = Object.assign({}, cfg.sections_control || {}, cfg.user_features || {});
...
function _applySectionsConfig() {
  const cfg = window._adminSectionsConfig || {};
  try { if(typeof _updateNavLocks === 'function') _updateNavLocks(); } catch(e) {}
  const ids = (_ORDO_SECTIONS.pages || []).map(x => x.id);
  Object.keys(cfg).forEach(id => { if(!ids.includes(id)) ids.push(id); });

  ids.forEach(id => {
    const state = window._getSectionAdminState(id);
    _collectSectionNav(id).forEach(el => {
      _setComingBadge(el, false);
      delete el.dataset.adminComing;
      if(state === 'disabled') {
        el.style.display = 'none';
        el.dataset.hiddenByAdmin = '1';
      } else {
        if(el.dataset.hiddenByAdmin === '1') el.style.display = '';
        delete el.dataset.hiddenByAdmin;
        if(state === 'coming') {
          el.style.display = '';
          el.dataset.adminComing = '1';
          el.setAttribute('aria-disabled', 'true');
          _setComingBadge(el, true);
        } else {
    ...
    _sectionAliases(id).forEach(alias => {
      const page = document.getElementById('page-' + alias);
      if(!page) return;
      delete page.dataset.adminHidden;
      delete page.dataset.adminComing;
      if(state === 'disabled') page.dataset.adminHidden = '1';
      if(state === 'coming') page.dataset.adminComing = '1';
    });
  });

}

if(!window.__ordoAdminSectionClickGuard) {
  window.__ordoAdminSectionClickGuard = true;
  document.addEventListener('click', function(e) {
    const item = e.target && e.target.closest ? e.target.closest('[data-admin-coming="1"]') : null;
    if(!item) return;
    e.preventDefault();
...
if(!window.__ordoAdminSectionShowGuard && typeof window.showPage === 'function') {
  const _origShowPageSections = window.showPage;
  window.__ordoAdminSectionShowGuard = true;
  window.showPage = function(id) {
    const state = window._getSectionAdminState ? window._getSectionAdminState(id) : 'enabled';
    if(state === 'disabled') {
      if(typeof toast === 'function') toast('القسم غير متاح حالياً');
      return false;
    }
    if(state === 'coming') {
      if(typeof toast === 'function') toast('القسم قريباً');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if(!window.__ordoSectionsConfigPoll) window.__ordoSectionsConfigPoll = setInterval(_loadSectionsConfig, 20000);
});
...
async function _loadAdminUpdatesFromCloud() {
  try {
    ...
    if(data?.config) {
      ...
      if(cfg.sections_control || cfg.user_features) {
        window._adminSectionsConfig = Object.assign({}, cfg.sections_control || {}, cfg.user_features || {});
        _applySectionsConfig();
      }
    }
...
if(typeof document !== 'undefined' && window.location.pathname.includes('admin')) {
  ...
  window.saveSectionsConfig = async function() {
    ...
    try {
      ...
      cfg.user_features = Object.assign({}, cfg.user_features || {});
      Object.keys(config).forEach(function(id) {
        cfg.user_features[id] = config[id] === false ? 'disabled' : (cfg.user_features[id] || 'enabled');
      });
...
(function(window) {
  ...
  function _ensureCurrencySettings(s){
    ...
    (s.settings.enabled_currencies || []).filter(function(cur){
...
(function () {
  ...
  window.openBudgetAlertDetails = function(projId) {
    var project = (_S().projects || []).find(function(p){ return String(p.id) === String(projId); });
    if (!project || typeof window.getProjectFinancialSummary !== 'function') return;
    var summary = window.getProjectFinancialSummary(projId);
    var invoices = typeof window.getProjectInvoices === 'function' ? window.getProjectInvoices(projId) : [];
    var tasks = typeof window.getProjectTasks === 'function' ? window.getProjectTasks(projId) : [];
    var cur = project.budgetCurrency || (typeof window._getCurrency === 'function' ? window._getCurrency() : 'ج.م');
    var money = function(v){ return Number(v || 0).toLocaleString() + ' ' + cur; };
    var pct = summary.budget ? Math.round((summary.totalInvoiced || 0) / summary.budget * 100) : 0;
    var rowsInv = invoices.length ? invoices.map(function(inv){
      var paid = inv.paid || inv.status === 'paid' || inv.status === 'مدفوعة';
      return '<tr><td>' + _esc(inv.num || inv.id || '-') + '</td><td>' + _esc(inv.date || inv.isoDate || '-') + '</td><td>' + money(inv.total) + '</td><td>' + (paid ? 'محصلة' : 'معلقة') + '</td></tr>';
    }).join('') : '<tr><td colspan="4" style="text-align:center;color:var(--text3);padding:14px">لا توجد فواتير مرتبطة</td></tr>';
    var rowsTasks = tasks.length ? tasks.map(function(t){
      var paid = t.paymentCollected || t.paid || t.paymentStatus === 'collected' || t.paymentStatus === 'paid';
      var invoiced = t.invoicePending || t.invoiced || t.paymentStatus === 'invoiced';
      return '<tr><td>' + _esc(t.title || 'مهمة') + '</td><td>' + money(t.value || t.amount || 0) + '</td><td>' + (paid ? 'محصلة' : (invoiced ? 'داخل فاتورة' : 'مستحقة')) + '</td></tr>';
    }).join('') : '<tr><td colspan="3" style="text-align:center;color:var(--text3);padding:14px">لا توجد مهام مشروع</td></tr>';

    var old = document.getElementById('_budget-detail-modal');
    if (old) old.remove();
    var ov = document.createElement('div');
    ov.id = '_budget-detail-modal';
    ov.className = 'modal-overlay';
    ov.style.cssText = 'display:flex;z-index:10020;padding:18px;background:rgba(0,0,0,.55)';
    ov.innerHTML =
      '<div class="modal" style="width:min(760px,96vw);max-height:88vh;overflow:auto">' +
        '<div class="modal-header"><div class="modal-title"><i class="fa-solid fa-triangle-exclamation" style="color:#f76f7c"></i> تفاصيل الميزانية - ' + _esc(project.name || 'مشروع') + '</div><button class="close-btn" onclick="this.closest(&quot;.modal-overlay&quot;).remove()"><i class="fa-solid fa-xmark"></i></button></div>' +
        '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:8px;margin-bottom:12px">' +
          '<div class="card" style="padding:12px"><small>الميزانية</small><b style="display:block;margin-top:4px">' + money(summary.budget) + '</b></div>' +
          '<div class="card" style="padding:12px"><small>إجمالي الفواتير</small><b style="display:block;margin-top:4px;color:var(--accent)">' + money(summary.totalInvoiced) + '</b></div>' +
          '<div class="card" style="padding:12px"><small>محصل</small><b style="display:block;margin-top:4px;color:var(--accent3)">' + money(summary.totalPaid) + '</b></div>' +
          '<div class="card" style="padding:12px"><small>معلق</small><b style="display:block;margin-top:4px;color:var(--accent4)">' + money(summary.totalUnpaid) + '</b></div>' +
          '<div class="card" style="padding:12px"><small>مصروفات</small><b style="display:block;margin-top:4px">' + money(summary.expenses) + '</b></div>' +
          '<div class="card" style="padding:12px"><small>المتبقي من الميزانية</small><b style="display:block;margin-top:4px;color:' + ((summary.budgetRemaining || 0) < 0 ? '#f76f7c' : 'var(--accent3)') + '">' + money(summary.budgetRemaining) + '</b></div>' +
        '</div>' +
        '<div style="height:10px;border-radius:999px;background:var(--surface2);overflow:hidden;margin-bottom:14px"><i style="display:block;height:100%;width:' + Math.min(100, Math.max(0, pct)) + '%;background:' + (summary.budgetExceeded ? '#f76f7c' : '#f7c948') + '"></i></div>' +
        '<div class="section-title" style="margin:10px 0 8px">الفواتير المرتبطة</div>' +
        '<div class="table-wrap"><table><thead><tr><th>رقم</th><th>التاريخ</th><th>الإجمالي</th><th>الحالة</th></tr></thead><tbody>' + rowsInv + '</tbody></table></div>' +
        '<div class="section-title" style="margin:14px 0 8px">تاسكات المشروع</div>' +
        '<div class="table-wrap"><table><thead><tr><th>المهمة</th><th>القيمة</th><th>الحالة</th></tr></thead><tbody>' + rowsTasks + '</tbody></table></div>' +
        '<div style="display:flex;gap:8px;margin-top:14px"><button class="btn btn-primary" onclick="this.closest(&quot;.modal-overlay&quot;).remove();showPage(&quot;projects&quot;);setTimeout(function(){ if(typeof openProjectDetail===&quot;function&quot;) openProjectDetail(&quot;' + _esc(project.id) + '&quot;); },120)">فتح المشروع</button><button class="btn btn-ghost" onclick="this.closest(&quot;.modal-overlay&quot;).remove()">إغلاق</button></div>' +
      '</div>';
    ov.addEventListener('click', function(e){ if(e.target === ov) ov.remove(); });
    document.body.appendChild(ov);
  };
  ...
  function _showBudgetAlerts() {
    ...
    alerts.slice(0, 3).forEach(function (item) {
      ...
      alert.onclick = function(){ window.openBudgetAlertDetails(project.id); };
  ...
  function patchCardAmounts(root){
    ...
    Array.prototype.slice.call(root.querySelectorAll('[data-task-id]')).forEach(function(card){
      ...
      Array.prototype.slice.call(card.querySelectorAll('span,div')).forEach(function(el){
        ...
        var looksLikeMoney = /\d/.test(compact) && /(EGP|USD|SAR|AED|ج\.?م|ر\.?س|\$)/.test(txt);
        if(looksLikeMoney || txt === String(task.value) || txt === Number(task.value).toLocaleString()){
...
(function(){
  ...
  function fixDom(root){
    ...
    Array.prototype.slice.call(root.querySelectorAll ? root.querySelectorAll('[placeholder],[title],[aria-label],option,label,button') : []).forEach(function(el){
      ...
      if(el.tagName === 'OPTION' || el.tagName === 'LABEL' || el.tagName === 'BUTTON'){
        Array.prototype.slice.call(el.childNodes || []).forEach(function(ch){
          if(ch.nodeType === 3) {
            var fixedText = fixText(ch.nodeValue);
            if(fixedText !== ch.nodeValue) ch.nodeValue = fixedText;
          }
        });
      }
    });
    ...
    if(tab) {
      var tabHtml = '<i class="fa-solid fa-wallet"></i> \u0627\u0644\u0639\u0645\u0644\u0627\u062a \u0648\u0627\u0644\u0645\u062d\u0627\u0641\u0638';
      if(tab.innerHTML !== tabHtml) tab.innerHTML = tabHtml;
    }
  }
  ...
  function hideRemovedSections(){
    Array.prototype.slice.call(document.querySelectorAll('#temp-todo-page-section, [id*="temp-todo"], [class*="temp-todo"]')).forEach(function(el){
      if(el.id === 'temp-todo-page-section' || /temp-todo/.test(el.id || '') || /temp-todo/.test(el.className || '')) el.style.display = 'none';
    });
  }
