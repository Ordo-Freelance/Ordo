(function(root){
  ...
  function publicPortalData(data,token){
    ...
    return {settings:publicSettings(data.settings),clients:(data.clients||[]).filter(function(x){return String(x.id)===clientId;}).map(function(x){return{id:x.id,name:x.name,email:x.email,phone:x.phone,company:x.company,currency_code:x.currency_code,currency_symbol:x.currency_symbol,openingBalance:x.openingBalance,openingBalanceCurrency:x.openingBalanceCurrency,openingBalanceType:x.openingBalanceType,openingBalanceNote:x.openingBalanceNote};}),projects:projects,project_tasks:(data.project_tasks||[]).filter(forClient),tasks:(data.tasks||[]).filter(forClient),invoices:(data.invoices||[]).filter(forClient),contracts:(data.contracts||[]).filter(forClient),proposals:(data.proposals||[]).filter(forClient),brief_forms:(data.brief_forms||[]).filter(function(x){return String(x.client_id)===clientId&&['sent','submitted','accepted'].includes(x.status);}).map(function(x){return{id:x.id,title:x.title,description:x.description,banner:x.banner,items:x.items,questions:x.questions,status:x.status,project_id:x.project_id,share_token:x.share_token};}),client_portals:(data.client_portals||[]).filter(function(x){return String(x.client_id||'')===clientId;}),svc_orders:(data.svc_orders||[]).filter(forClient)};
  }
