(function(){
  ...
  function mount(el,options){
    ...
    el.querySelector('.pc-messages').onclick=e=>{const button=e.target.closest('[data-pc-delete]');if(button)deleteMessage(el,button.dataset.pcDelete);};
  ...
  async function send(el,attachment){const s=state.get(el),field=el.querySelector('[data-pc-text]'),body=field.value.trim(),button=el.querySelector('[data-pc-send]');if(!body&&!attachment)return false;button.disabled=true;
  ...
  async function deleteMessage(el,id){const s=state.get(el);if(!window.confirm('حذف هذه الرسالة والمرفق المرتبط بها لدى الطرفين؟'))return;
    try{await api(s,'delete',{message_id:id});s.media.delete(id);await refresh(el);}catch(e){el.querySelector('.pc-error').textContent=e.message||'تعذر حذف الرسالة';}}
  ...
  async function voice(el){const s=state.get(el);if(s.recorder?.state==='recording')return;if(!navigator.mediaDevices?.getUserMedia||!window.MediaRecorder){el.querySelector('.pc-error').textContent='المتصفح لا يدعم تسجيل الصوت';return;}
    ...
    try{const stream=await navigator.mediaDevices.getUserMedia({audio:true});const rec=new MediaRecorder(stream,{mimeType:mime,audioBitsPerSecond:24000});s.recorder=rec;s.chunks=[];s.cancelRecording=false;rec.ondataavailable=e=>{if(e.data.size)s.chunks.push(e.data);};
      rec.onstop=()=>{clearInterval(s.recordTimer);cancelAnimationFrame(s.waveFrame);s.audioContext?.close();s.audioContext=null;stream.getTracks().forEach(t=>t.stop());el.querySelector('.pc-recording').hidden=true;s.recorder=null;if(s.cancelRecording)return;const blob=new Blob(s.chunks,{type:rec.mimeType});if(blob.size>1500000){el.querySelector('.pc-error').textContent='التسجيل تجاوز الحد المسموح';return;}showVoicePreview(el,blob);};
  ...
  function showVoicePreview(el,blob){const s=state.get(el),box=el.querySelector('.pc-preview'),url=URL.createObjectURL(blob);s.preview={blob,url};
    ...
    box.querySelector('[data-pc-send-voice]').onclick=async e=>{e.currentTarget.disabled=true;const data=await new Promise(resolve=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.readAsDataURL(blob);});const sent=await send(el,{kind:'voice',mime:blob.type.split(';')[0],name:'تسجيل صوتي',data});if(sent)clearPreview(el);else e.currentTarget.disabled=false;};}
